...
module.exports = grammar({
  ...
  rules: {
    ...
    _command_nocomment: ($) => choice($._builtin, $.command_nocomment),
    ...
    command_nocomment: ($) =>
      seq(
        field("name", $._word),
        optional(field("arguments", $.word_list_nocomment)),
      ),

    word_list: ($) => repeat1($._word),
    word_list_nocomment: ($) => repeat1($._word_nocomment),
    ...
    _word_nocomment: ($) =>
      seq(optional($.unpack), choice($.braced_word_simple, $._concat_word)),
    ...
    command_substitution: ($) => seq("[", $._command_nocomment, "]"),
